...
export function inferKind(fieldType: string, fieldName: string): ReviewCheckKind {
...
function formatFieldValue(
  ...
  if (kind === "address" && fieldName === "verifyingContract" && tokenHint) {
    return {
      displayValue: `${tokenHint.symbol} (${shortenAddress(rawValue)})`,
    };
  }
...
function fieldLabel(group: string, fieldName: string, pathLabel?: string): string {
  ...
  if (group === "domain") {
    return DOMAIN_LABELS[fieldName] ?? capitalize(fieldName);
  }
  if (group === "message") {
    return MESSAGE_LABELS[fieldName] ?? capitalize(fieldName);
  }
...
export function buildFieldCheck(opts: {
  id: string;
  group: string;
  fieldName: string;
  fieldType: string;
  ...
  const check: ReviewCheckItem = {
    id: opts.id,
    group: opts.group,
    label: fieldLabel(opts.group, opts.fieldName, opts.pathLabel),
    value,
    kind,
  };

  if (displayValue !== undefined) check.displayValue = displayValue;
  return check;
}
